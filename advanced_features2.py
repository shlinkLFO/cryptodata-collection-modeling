...
def create_advanced_features_table(engine):
    ...
    # Target column is now 'target_1h_direction' (Integer)
    features_table = Table(
        ...
        # Target column for prediction (Changed!)
        # Represents the direction: 1 for positive return, 0 for non-positive (negative or zero)
        # Allows NULLs for the last row where future is unknown
        Column('target_1h_direction', Integer, nullable=True)
    )
...
def calculate_advanced_features(df):
    ...
    df = df.copy()
    df['timestamp'] = pd.to_datetime(df['timestamp'])
    # Set index for calculations, but keep timestamp column
    df = df.sort_values('timestamp').set_index('timestamp', drop=False)

    # --- Feature Calculations (Keep all feature calculations as they were) ---
    ...
    # MAs
    min_periods_base = 2
    for hours in [3, 6, 12, 24, 48, 72, 168]: df[f'ma_{hours}h'] = df['close'].rolling(window=hours, min_periods=min_periods_base).mean()
    # Stds
    for hours in [3, 6, 12, 24, 48, 72, 168]: df[f'rolling_std_{hours}h'] = df['close'].rolling(window=hours, min_periods=min_periods_base).std()
    ...
    for hours in [14, 24, 48]: df[f'atr_{hours}h'] = df['tr'].rolling(hours, min_periods=hours).mean()
    # Momentum Indicators
    delta = df['close'].diff()
    ...
    for hours in [12, 24, 72, 168]: df[f'volume_ma_{hours}h'] = df['volume_btc'].rolling(window=hours, min_periods=min_periods_base).mean()
    ...
    # --- Target Calculation (Modified!) ---
    # 1. Calculate the actual future 1-hour pct return correctly
    #    Return = (Price[t+1] - Price[t]) / Price[t]
    future_return = (df['close'].shift(-1) - df['close']) / df['close']

    # 2. Create binary target: 1 if future return is positive, 0 otherwise (non-positive)
    #    Result will be float (1.0, 0.0) or NaN. Convert to Integer later during insertion prep.
    df['target_1h_direction'] = np.where(future_return > 0, 1, 0)
    # The last row will have NaN because shift(-1) is NaN

    # --- Cleanup and Selection ---
    df = df.reset_index(drop=True) # Drop the temporary timestamp index
    df = df.replace([np.inf, -np.inf], np.nan)

    # Define the list of columns to keep (including the new target column)
    columns = [
        ...
        'target_1h_direction' # <<< Updated target column name
    ]
...
def insert_features_batch(engine, features_df):
    ...
    # --- Minor change: Ensure target column is handled as object if it contains None ---
    ...
    # Add target_1h_direction to int_cols check, as it should be Integer/None
    int_cols = [f'hour_{h}' for h in range(24)] + [f'day_{d}' for d in range(7)] + ['target_1h_direction']
    for col in int_cols:
        if col in features_df.columns:
             features_df[col] = features_df[col].astype('object') # Handles None
    ...
    try:
        ...
        if data_tuples: print(f"Sample data row (first 10 elements): {str(data_tuples[0][:10])}")
        traceback.print_exc()
        return 0
    finally:
...
def process_symbol_data_incremental(engine, symbol, required_overlap_hours):
    ...
    if raw_df.empty:
        print(f"No OHLCV data found for {symbol} in the required range.")
        return 0
    print(f"Fetched {len(raw_df)} raw OHLCV rows.")
    raw_df = raw_df.sort_values('timestamp').reset_index(drop=True)

    if latest_processed_ts and not raw_df.empty and raw_df['timestamp'].max() <= latest_processed_ts:
        print(f"No OHLCV data strictly newer than {latest_processed_ts} found.")
        return 0
    ...
    if features_df.empty:
        print("Feature calculation resulted in empty DataFrame.")
        return 0
    ...
    if num_to_insert > 0:
        ...
        print(f"Calculated {len(features_df)} feature rows, but no timestamps were newer than {latest_processed_ts}.")
        return 0
...
def main():
    ...
    try:
        ...
        create_advanced_features_table(engine) # Will create/update table with new target column
        print("Table check complete.")
    except Exception as e:
        print(f"[{datetime.now()}] CRITICAL ERROR: Failed to connect/ensure table exists. Exiting.")
        print(f"Error: {e}")
        traceback.print_exc()
        return
...
if __name__ == "__main__":
    try:
        ...
        print(f"\n[{datetime.now()}] An unexpected critical error occurred:")
        print(f"Error: {e}")
        traceback.print_exc()